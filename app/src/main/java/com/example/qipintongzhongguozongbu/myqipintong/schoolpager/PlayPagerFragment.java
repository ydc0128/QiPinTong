...
/**
 * Description: 易通学院娱乐页面
 * Copyright  : Copyright (c) 2016
 * Author     : 感觉自己懵懵哒
 * E-mail     :anber1229423614@163.com
 * Date       : 2017/2/8 下午5:32
 */
public class PlayPagerFragment extends BaseFragment {
    @BindView(R.id.bb_play)
    Banner bbPlay;
    @BindView(R.id.rv_play)
    RecyclerView rvPlay;
    @BindView(R.id.rf_play)
    PullToRefreshLayout rfPlay;
    Unbinder unbinder;

    @Override
    public View initView() {
        return swipeBackView(View.inflate(mActivity, R.layout.fragment_play, null));
    }


    public static BaseFragment getInstance() {
        return new PlayPagerFragment();
    }

    @Override
    public void onLazyInitView(@Nullable Bundle savedInstanceState) {

        setRefreshDate(rfPlay);

        setBannerImage(bbPlay);
    ...
    private void setRecyclerViewAdapter() {
        ...
        rvPlay.setLayoutManager(new LinearLayoutManager(mActivity));

        rvPlay.setAdapter(new NewsPagerAdapter(R.layout.item_news, ImageList.getImageList()));
        //添加头布局

        rvPlay.addOnItemTouchListener(new OnItemClickListener() {
            @Override
