 ...
 * Date       : 2017/3/4 下午5:50
 */
public class FashionPagerFragment extends BaseFragment {
    @BindView(R.id.bb_fashion)
    Banner bbFashion;
    @BindView(R.id.rv_fashion)
    RecyclerView rvFashion;
    @BindView(R.id.rf_fashion)
    PullToRefreshLayout rfFashion;
    Unbinder unbinder;

    @Override
    public View initView() {
        return swipeBackView(View.inflate(mActivity, R.layout.fragment_pager_fashion, null));
    }


    public static BaseFragment getInstance() {
        return new FashionPagerFragment();
    }

    @Override
    public void onLazyInitView(@Nullable Bundle savedInstanceState) {

        setRefreshDate(rfFashion);

        setBannerImage(bbFashion);
    ...
    private void setRecyclerViewAdapter() {
        ...
        rvFashion.setLayoutManager(new LinearLayoutManager(mActivity));

        rvFashion.setAdapter(new NewsPagerAdapter(R.layout.item_news, ImageList.getImageList()));
        //添加头布局

        rvFashion.addOnItemTouchListener(new OnItemClickListener() {
            @Override
